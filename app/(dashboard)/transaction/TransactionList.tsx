...
const PurchaseList = () => {
  ...
  const getBundleData = async () => {
    ...
    try {
      ...
      const uri = new URL(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/bundle/dropdown`
      );
  ...
  const getOperatorData = async () => {
    ...
    try {
      const token = await bearerToken();
      const uri = new URL(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/operator/dropdown`
      );
  ...
  const getData = async () => {
    ...
    try {
      ...
      let response = res.data?.data?.original?.data;
      ...
      <div className="shadow-md p-2 border  rounded-md mb-4">
        <div className="flex justify-between items-center">
          ...
          <div className=" flex gap-3 items-center">
            ...
            <Select
              value={operator_id}
              onValueChange={(field) => setOperator_id(field)}
            >
              <SelectTrigger>
            ...
            <Select
              value={bundle_id}
              onValueChange={(field) => setBundle_id(field)}
            >
              <SelectTrigger>
